...
namespace WPE\Atlas\Headless\Extension\Blueprint\Fetch;
...
function get_local_blueprint( string $path ) {
	if ( ! is_readable( $path ) ) {
		return new WP_Error(
			'wpe_atlas_headless_extension_blueprint_file_not_readable',
			esc_html__( 'File or directory not found or readable.', 'wpe-atlas-headless-extension' )
		);
	}
...
function get_remote_blueprint( string $url ) {
	if ( ! filter_var( $url, FILTER_VALIDATE_URL ) ) {
		return new WP_Error(
			'wpe_atlas_headless_extension_blueprint_invalid_url',
			esc_html__( 'Please provide a URL to a blueprint zip file.', 'wpe-atlas-headless-extension' ),
			[ 'status' => 400 ]
		);
	}
	...
	if ( is_wp_error( $response ) ) {
		return new WP_Error(
			'wpe_atlas_headless_extension_blueprint_http_error',
	...
	if ( 200 !== wp_remote_retrieve_response_code( $response ) ) {
		return new WP_Error(
			'wpe_atlas_headless_extension_blueprint_http_error_response_code',
			sprintf(
	...
	if ( empty( $zip_file ) ) {
		return new WP_Error(
			'wpe_atlas_headless_extension_blueprint_http_error_empty_body',
			esc_html__( 'Received empty response body.', 'wpe-atlas-headless-extension' )
		);
	}
...
function save_blueprint_to_upload_dir( string $blueprint, string $filename ) {
	...
	if (
		...
		if ( ! $wp_filesystem->is_dir( $blueprint ) ) {
			return new WP_Error(
				'wpe_atlas_headless_extension_blueprint_save_error',
				/* translators: path to blueprint */
				sprintf( esc_html__( 'Could not read directory at %s', 'wpe-atlas-headless-extension' ), $blueprint )
			);
		}
		...
		if ( ! $copied ) {
			return new WP_Error(
				...
				sprintf( esc_html__( 'Error copying directory from %1$s to %2$s', 'wpe-atlas-headless-extension' ), $blueprint, $destination )
			);
		}
	...
	if ( ! $saved ) {
		return new WP_Error(
			...
			sprintf( esc_html__( 'Error saving temporary file to %s', 'wpe-atlas-headless-extension' ), $temp_destination )
		);
	}
	...
	if ( mime_content_type( $temp_destination ) !== 'application/zip' ) {
		...
		return new WP_Error(
			'wpe_atlas_headless_extension_blueprint_unsupported_file_type',
			esc_html__( 'Provided file type is not supported. Please provide a link to a valid zip file.', 'wpe-atlas-headless-extension' )
		);
	}
	...
	if ( ! $result ) {
		...
		return new WP_Error(
			...
			sprintf( esc_html__( 'Error saving file to %s', 'wpe-atlas-headless-extension' ), $destination )
		);
	}
